<template>
  <v-container fill-height fluid class="ma-0 pa-0">
    ...
    <div
      class="blackout"
      v-bind:style="{ display: displayValue, opacity: opacityValue }"
    >
      <draggable
        v-model="items1"
        ref="dropArea"
...
export default {
  ...
  updated: function () {
    this.$nextTick(function () {
      ...
      this.$refs.gridLayer.getNode().removeChildren();
      var padding = this.gridSize;
      console.log(
        this.stageSize.width,
        padding,
        this.stageSize.width / padding
      );
      for (var i = 0; i < this.stageSize.width / padding; i++) {
        this.$refs.gridLayer.getNode().add(
          new Konva.Line({
            points: [
      ...
      this.$refs.gridLayer.getNode().draw();
    });
  },
  methods: {
    onSpill(event, str) {
    ...
    startCreating(event) {
      ...
      if (this.$store.state.event.type == operations.CREATE) {
        ...
        this.createComponent(
          this.$store.state.event.primitive,
          event.evt.layerX,
          event.evt.layerY
        );
      }
      ...
      if (this.$store.state.event.type == operations.CREATING) {
        ...
        if (Math.abs(initWidth) >= 2 || Math.abs(initHeight) >= 2) {
          if (this.$store.state.event.createdPrimitve === null) {
            this.$store.state.event.createdPrimitve = this.createComponent(
              this.$store.state.event.primitive,
              startX,
              startY
            );
